...
const Editor = ({ text, updateText, setInfoMessages, setCurrentComponent }) => {
  ...
  useEffect(() => {
    if (selectedText) {
      const sentencesOfSpanElements = createSpanArray(
        selectedText,
        handleWordClick,
        "editor__word",
        "editor__word--target",
  ...
  const handleWordClick = (e) => {
    ...
    if (e.target.classList.contains("editor__word--target")) {
      wordObj = selectedText.targetWordObjs.filter(
        (obj) => obj.element === elementIndex && obj.sentence === sentenceIndex
      )[0];
  ...
  async function handleSubmit(e) {
    ...
    //is this inefficient?
    const res = await axios.post("/api/getWordData", {
    ...
    if (wordIsSaved) {
      ...
      updatedWordArr = selectedText.targetWordObjs.map((obj) => {
        if (
          obj.sentence === selectedWordObj.sentence &&
          obj.element === selectedWordObj.element
        ) {
          obj = updatedWordObj;
        }
        return obj;
  ...
  const handleDictionaryClick = () => {
    axios
      .post("/api/getWordData", { word: selectedWordObj.word })
      .then((res) => {
        const {
          definition,
          audio,
  ...
  const handleDelete = () => {
    const updatedWordArr = selectedText.targetWordObjs.filter(
      (obj) =>
        obj.sentence !== selectedWordObj.sentenceIndex ||
        obj.element !== selectedWordObj.elementIndex
    );
    const updatedText = {
      ...selectedText,
      targetWordObjs: updatedWordArr,
